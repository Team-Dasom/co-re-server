...
import static com.dongyang.core.domain.gpt.constant.GptConstant.GPT_ADD_COMMENT_MODEL;
import static com.dongyang.core.domain.gpt.constant.GptConstant.GPT_DEFAULT_MODEL;
import static com.dongyang.core.domain.gpt.constant.GptConstant.GPT_RECOMMEND_VARIABLE_NAME_MODEL;
...
public enum GptFunction {
    ...
    REFACTOR_CODE(
            "당신은 %s 언어로 작성된 코드를 받으면 %s의 클린코드 규칙을 학습 후, 리팩토링해주는 코드 전문가이다. 만약 받은 코드가 %s 언어의 문법이 아니면 X를 출력하고, 받은 코드가 %s 언어로 작성된 것이 맞다면 받은 %s 언어의 클린코드 규칙을 학습 후, 코드를 리팩토링해서 보여줘",
            GPT_DEFAULT_MODEL, 0.4, 2000),
    ;
